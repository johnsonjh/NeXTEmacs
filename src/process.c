...
#if defined(USG) && !defined(IBMRTAIX) && !defined(_h_BSDTYPES)
#include <time.h>
     ...
     WAITTYPE w;
{
  if (WIFSTOPPED (w))
    ...
    return Fcons (Qexit, Fcons (make_number (WSTOPSIG (w)),
				WCOREDUMP (w) ? Qt : Qnil));
  else if (WIFSIGNALED (w))
    return Fcons (Qsignal, Fcons (make_number (WSTOPSIG (w)),
				  WCOREDUMP (w) ? Qt : Qnil));
  else
    return Qrun;
}
...
/* Open an available pty, returning a file descriptor.
   ...
   is left in the variable ptyname.  */
...
allocate_pty ()
{
  ...
  for (c = FIRST_PTY_LETTER; c <= 'z'; c++)
	...
	if (stat (ptyname, &stb) < 0)
	  return -1;
	fd = open (ptyname, O_RDWR | O_NDELAY, 0);
