...
; begin NeXT
	(init (if noninteractive nil ("")))
	(done nil))
     ;; If user has sued to "root", do not use the HOME environment variable.
     (and init 
 	 (= (user-uid) 0) (not (string= (user-login-name) "root"))
 	 (setq init (user-login-name)))
; end NeXT
